...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        self.product_recurrente_checkout = payment_link_data["metadata"]["product"]
    ...
    def _get_tx_from_webhook_data(self, notification_data):
        ...
        if not tx:
            raise ValidationError(
                "Recurrente: " + _("No transaction found matching reference %s and %s." % (checkout_id, product_id))
            )
        return tx
    ...
    def _process_webhook_data(self, notification_data):
        ...
        if payment_status in const.PAYMENT_STATUS_MAPPING['pending']:
            ...
            self._set_error(_(
                "An error occurred during the processing of your payment (status %s). Please try again.\nReason: %s" % (payment_status, failure_reason),
            ))
        else:
            _logger.warning(
                _("Received data with invalid payment status (%s) for transaction with reference %s." % (payment_status, self.reference)),
            )
            self._set_error("Recurrente: " + _("Unknown payment status: %s" % payment_status))
